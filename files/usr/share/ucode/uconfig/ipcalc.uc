 ...
 */
export function convert_bits_to_mask(bits, v6) {
	let width = v6 ? 128 : 32,
	    mask = [];
...
export function apply_mask(addr, mask) {
...
export function generate_prefix(state, template, ipv6) {
	...
	if (prefix && prefix[1] == 'auto') {
		assert(state.definitions && state.definitions[ipv6 ? 'ipv6_network' : 'ipv4_network'],
			"No global prefix pool configured");

		let pool = match(state.definitions[ipv6 ? 'ipv6_network' : 'ipv4_network'], /^([0-9a-fA-F:.]+)\/([0-9]+)$/);
